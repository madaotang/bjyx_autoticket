...
class Concert(object):
    ...
    def get_cookie(self):
        ...
        while self.driver.title == '大麦登录':  # 等待扫码完成
            sleep(1)
        dump(self.driver.get_cookies(), open("damai_cookies.pkl", "wb"))
        print("###Cookie保存成功###")

    def set_cookie(self):
        try:
            cookies = load(open("damai_cookies.pkl", "rb"))  # 载入cookie
    ...
    def login(self):
        if not exists('damai_cookies.pkl'):  # 如果不存在cookie.pkl,就获取一下
    ...
    def enter_concert(self):
        ...
        try:
            ...
            self.status = 0
            self.driver.quit()
            raise Exception("***错误：登录失败,请检查配置文件昵称或删除damai_cookies.pkl后重试***")
